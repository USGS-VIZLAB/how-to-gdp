visualize.precip_map <- function(viz){
  library(ggplot2)
  ...
  precip_df_final <- precip_df %>% filter(DateTime == max(precip_df$DateTime))
  
  precip_df_sp_final <- left_join(storm_cell_df, precip_df_final, by="id") # merge w/ precip totals
